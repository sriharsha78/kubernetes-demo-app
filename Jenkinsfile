pipeline {
	...
	stages {
		...
		stage ('app-calc-deploy - Checkout') {
			steps {
					checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/ViniciusSouza/app-calc-deploy']]])
			}
		}
		...
		stage ('Helm Deploy to K8s'){
			steps{
                	...
                	#HELM config
					NAME="app-calc-deploy"
					HELM_CHART="./helm/app-calc-deploy"
